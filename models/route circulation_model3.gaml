/**
 *  routecirculation - model 3
 *  Author: david
 *  Description: 
 */
model routecirculationmodel3gaml
...
//Espece route
	...
	species moto skills: [moving]
	{
		...
		int taille_moto <- 7;
		bool moto_en_attente <- false;
		bool moto_roulant <- false;
		aspect basic
		{
		...
		reflex circulation_feux_rouge_et_feux_vert
		{
			ask feux
			{
				...
				if ((self.color = # orange) and (distance <= 27.0))
				{
					...
					myself.moto_en_attente <- true;
					myself.moto_roulant <- false;
				}
				...
				if ((self.color = # green) and (distance <= 27.0))
				{
					...
					myself.moto_en_attente <- false;
					myself.moto_roulant <- true;
				}
...
experiment routecirculation type: gui
{
	...
	output
	{
		...
		display Stats_Circulation background : # magenta
		{
			chart "Statistiques des Agents" type: pie
			{
				data "Moto(s) En Attente" value: moto count (each.moto_en_attente) color: # red;
				data "Periode Temps" value: feux count (each.temps_total) color: # yellow;
				data "Motos qui roulent" value: moto count (each.moto_roulant) color: # green;
			}
